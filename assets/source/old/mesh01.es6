...
class Mesh {
    ...
    addMidPoint ( idx0, idx1, vertexArr, midArr ) {
        ...
        let w1 = 3 / 8; // How to weigh Vertices which created a midpoint

        let w2 = 1 / 8; // How to weigh Vertices at the 3nd point of the original triangle
        ...
        if ( midArr[ revKey ] ) { // only the revKey shows up as previously present
            ...
            idx = midArr[ key ];
                ...
                v0.addPrevVertex( m0, w1 );

                v1.addNextVertex( m0, w1 );

                // Add .prev and .next to the new midpoint (weight 3/8), also .control.

                m0.addPrevVertex( v0, w1 );

                m0.addNextVertex( v1, w1 );

                // Get the third set of Vertices between v0 and v1 (forming a triangle)

                m0.addControlVertex( v1.next );

                m0.addControlVertex( v1.prev );

                m0.addControlVertex( v0.prev );

                m0.addControlVertex( v0.next );

                // Clone a copy of the midpoint into the tempMidpointArr (needed for smoothing)

                this.tempMidArr[ m0.idx ] = m0.clone();
    ...
    subdivide () {
        ...
        for ( let i = 0; i < vertexArr.length; i++ ) {
            ...
            let vtx = this.vertexArr[ i ];

            if ( vtx.isOdd ) {

                if ( vtx.prev.length > 1 ) console.error('ODD TOO BIG:' + vtx.prev.length)

                if ( vtx.next.length > 1 ) console.error('ODD TOO BIG:' + vtx.next.length)
    ...
    uniqueify () {
        ...
        console.warn( 'mesh::uniqueify(): Note: mesh optimized from:' + oldVertexArr.length + ' to:' + vertexArr.length + ' vertices' );
    ...
    smooth () {
        ...
        if ( this.notSmoothed ) {
            ...
            for ( let i = 0; i < vertexArr.length; i++ ) {
                ...
                let coords = vtx.coords;

                // this.tempMidArr has clones of the midpoints for the second adjustment step, indexed with same idx
                ...
                if ( vtx.isOdd ) {
                    ...
                    // Compute smoothing from old midpoint array, all points contribute

                    vtx.adjustEven();

                    console.log("EVEN control length:" + vtx.control.length)
